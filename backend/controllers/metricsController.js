...
exports.getMetrics = async (req, res) => {
    try {
        ...
        const accuracyRate =
            totalFeedback > 0 ? parseFloat((accurateFeedback / totalFeedback).toFixed(4)) : null;
        ...
        const avgConfidence =
            totalScans > 0
                ? parseFloat(
                      (
                          userScans.reduce((sum, scan) => sum + scan.confidence, 0) / totalScans
                      ).toFixed(4),
                  )
                : null;
        ...
        res.status(500).json({ error: 'Metrics server error' });
    }
};
